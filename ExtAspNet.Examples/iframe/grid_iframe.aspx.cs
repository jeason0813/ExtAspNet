    ...
    public partial class grid_iframe : PageBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //string confirmScript = ExtAspNet.Confirm.GetShowReference("页面数据发生变化，是否先保存？", "确认关闭", ExtAspNet.MessageBoxIcon.Question, "alert('需要先保存数据。');", "alert('不保存数据。');");
                //btnSimulateClose.OnClientClick = String.Format("if({0}){{{1}}}else{{alert('页面数据，没有变化');}}", ExtAspNet.PageContext.GetPageStateChangedReference(), confirmScript);
                ...
                ////string confirmScript2 = ExtAspNet.Confirm.GetShowReference(confirmText, "确认关闭", ExtAspNet.MessageBoxIcon.Warning, closeScript, "return false;");
